...
<%
    ...
    // 인자받기
    String f_name = request.getParameter("f_name"); // 다운로드 할 파일명
    ...
    if(f.exists()) { // 존재할때만 다운로드 해야하니까 존재여부를 물어봄
        ...
        ServletOutputStream sos = null; // **************** 요청자에게
        // 응답으로 스트림을 줘야 다운로드가 된다. - response를 통해 얻을 수 있는 output스트림이
        ...
        try{
            ...
            while((size = bis.read(buf)) != -1){ // bis로부터 읽어와 buf에 쌓아둔다.
                // 읽은 자원은 buf에 저장되며, 읽은 바이트 수는 size가 기억하고 있다.
