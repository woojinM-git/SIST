    ...
    <table summary="게시판 목록">
        ...
        <tr>
            <td colspan="4">
                <ol class="paging">
                    ...
                    <c:if test="${p.startPage >= p.pagePerBlock}">
                        <li><a href="/list?cPage=${p.nowPage-p.getPagePerBlock}">&lt;</a></li>
                    </c:if>
                    ...
                    <c:forEach begin="${p.startPage}" end="${p.endPage}" varStatus="vo">
                        ...
                        <c:if test="${p.nowPage != vo.index}">
                            <li><a href="/list?cPage=${vo.index}">${vo.index}</a></li>
                        </c:if>
                    </c:forEach>

                    <c:if test="${p.endPage < p.getTotalPage()}">
                        <li><a href="/list?cPage=${p.nowPage+p.pagePerBlock}&bname=${bname}">&gt;</a></li>
                    </c:if>
            ...
            <td>
                <input type="button" value="글쓰기"
                       onclick="javascript:location.href='/write?cPage=${p.nowPage}&bname=${bname}'"/>
            </td>
        </tr>
        </tfoot>
        <tbody>
        ...
        <tr>
            ...
            <td style="text-align: left">
                <a href="view?bname=BBS&b_idx=${vo.b_idx}&cPage=${p.nowPage}">
                    ${vo.subject}
