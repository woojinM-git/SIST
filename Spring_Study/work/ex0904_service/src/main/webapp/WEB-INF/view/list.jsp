...
<c:if test="${p.startPage >= p.pagePerBlock}">
    ...
    <c:forEach begin="${p.startPage}" end="${p.endPage}" varStatus="vs">
        ...
        <c:if test="${p.nowPage != vs.index}">
          <li><a href="/list?cPage=${vs.index}&bname=${bname}">${vs.index}</a></li>
        </c:if>
    </c:forEach>
    ...
    <c:if test="${p.endPage >= p.totalPage}">
      ...
      <td>
        <input type="button" value="글쓰기"
           onclick="javascript:location.href='/write?cPage=${page.nowPage}&bname=${bname}'"/>
      </td>
    </tr>
    </tfoot>
    <tbody>
    ...
    <tr>
      ...
      <td style="text-align: left">
        <a href="/view?bname=${bname}&b_idx=${vo.b_idx}&cPage=${p.nowPage}">
          ${vo.subject}
